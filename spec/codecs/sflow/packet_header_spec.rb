...
describe EthernetHeader do
  it "should decode ipv4 over tcp header" do
    payload = IO.read(File.join(File.dirname(__FILE__), "ethernet_ipv4_over_tcp_header.dat"), :mode => "rb")
    decoded = EthernetHeader.new(:size_header => payload.bytesize * 8).read(payload)

    decoded["eth_dst"].to_s.should eq("00:00:0c:07:ac:01")
    decoded["eth_src"].to_s.should eq("00:23:e9:78:16:c6")
    decoded["eth_type"].to_s.should eq("2048")
    decoded["eth_data"]["header"]["dst_ip"].to_s.should eq("10.243.2.1")
    decoded["eth_data"]["header"]["layer4"]["dst_port"].to_s.should eq("8140")
  end
end